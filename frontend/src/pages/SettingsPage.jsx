...
const SettingsPage = () => {
  ...
  const handleViewMyData = async () => {
    try {
      setLoading(true);
      const response = await axiosClient.get(`/api/gdpr/users/${user.id}/data-access`);
    ...
    return (
      <div className="space-y-4">
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          ...
          <p><strong>Användarnamn:</strong> {data.profile?.username}</p>
          <p><strong>Email:</strong> {data.profile?.email}</p>
          <p><strong>Skapat:</strong> {new Date(data.profile?.createdAt).toLocaleDateString('sv-SE')}</p>
        </div>
        
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h4 className="font-semibold text-lg mb-2">Yaps ({data.yaps?.length || 0})</h4>
            ...
            <div className="max-h-40 overflow-y-auto space-y-2">
              {data.yaps.map((yap, index) => (
                <div key={index} className="text-sm border-b pb-1">
                  ...
                  <p className="text-gray-500 text-xs">{new Date(yap.createdAt).toLocaleDateString('sv-SE')}</p>
                </div>
              ))}
            </div>
          ) : (
        ...
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h4 className="font-semibold text-lg mb-2">Gillningar ({data.likes?.length || 0})</h4>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <h4 className="font-semibold text-lg mb-2">Svar ({data.replies?.length || 0})</h4>
          {data.replies && data.replies.length > 0 ? (
            <div className="max-h-40 overflow-y-auto space-y-2">
              {data.replies.map((reply, index) => (
                <div key={index} className="text-sm border-b pb-1">
                  <p className="truncate">{reply.content}</p>
                  <p className="text-gray-500 text-xs">{new Date(reply.createdAt).toLocaleDateString('sv-SE')}</p>
                </div>
              ))}
            </div>
          ) : (
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center">
        <p className="text-lg text-gray-600 dark:text-gray-300">Du måste vara inloggad för att komma åt inställningar.</p>
      </div>
    );
  }
          ...
          <div className="space-y-8">
            ...
            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-6">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4">
                Enligt GDPR Artikel 15 har du rätt att få tillgång till all personlig data vi har om dig.
              </p>
              <button
            ...
            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-6">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-4">
                Enligt GDPR Artikel 20 har du rätt att få dina data i ett portabelt format.
              </p>
              ...
              >
                {loading ? 'Exporterar...' : `Exportera som ${exportFormat.toUpperCase()}`}
              </button>
            </div>
            ...
            <div className="border border-red-200 dark:border-red-600 rounded-lg p-6 bg-red-50 dark:bg-red-900/20">
              ...
              <p className="text-red-600 dark:text-red-400 mb-4">
                Enligt GDPR Artikel 17 har du "rätten att bli glömd". Detta tar bort ALL din data permanent.
              </p>
              ...
              {!showDeleteConfirmation ? (
                ...
                <div className="space-y-4">
                  <div className="bg-red-100 dark:bg-red-800 p-4 rounded-lg border border-red-300 dark:border-red-600">
                    <h4 className="font-semibold text-red-800 dark:text-red-200 mb-2">⚠️ VARNING</h4>
                    <p className="text-red-700 dark:text-red-300 text-sm mb-2">
                      Denna åtgärd kan INTE ångras! All din data kommer att tas bort permanent:
                    </p>
                    <ul className="text-red-700 dark:text-red-300 text-sm list-disc ml-6">
                      ...
                      <li>Alla dina gillningar och interaktioner</li>
            ...
            <div className="border border-gray-200 dark:border-gray-600 rounded-lg p-6 bg-blue-50 dark:bg-blue-900/20">
              ...
              <div className="text-blue-700 dark:text-blue-300 space-y-2">
                <p><strong>Artikel 15:</strong> Rätt till tillgång - Du kan begära kopior av dina personuppgifter.</p>
                <p><strong>Artikel 17:</strong> Rätten att bli glömd - Du kan begära att vi raderar dina personuppgifter.</p>
                <p><strong>Artikel 20:</strong> Dataportabilitet - Du kan begära dina data i ett maskinläsbart format.</p>
                <p><strong>Artikel 32:</strong> Säkerhet - Vi implementerar lämpliga säkerhetsåtgärder för att skydda dina data.</p>
              </div>
            </div>
          </div>
        </div>
